...
describe("receipt utilities", () => {
  ...
  it("formats receipt share text with the Explorer URL", () => {
    const explorerUrl = "https://explorer.solana.com/tx/5abc?cluster=devnet";

    assert.equal(
      formatReceiptShareTextWithUrl(
        "Paid with KonekPay. Verified on Solana Devnet: {{url}}",
        explorerUrl,
        "Paid with KonekPay. Verified on Solana Devnet.",
      ),
      `Paid with KonekPay. Verified on Solana Devnet: ${explorerUrl}`,
    );
    ...
    assert.equal(
      formatReceiptShareTextWithUrl(
        ...
        explorerUrl,
        "Dibayar lewat KonekPay. Terverifikasi di Solana Devnet.",
      ),
      `Dibayar lewat KonekPay. Terverifikasi di Solana Devnet: ${explorerUrl}`,
    );
  });
  ...
  it("humanizes payment and settlement enum labels for receipt UI", () => {
    ...
    assert.equal(
      humanizePaymentLabel("SETTLED_SIMULATED"),
      "Settlement simulated",
    );
    assert.equal(
      humanizePaymentLabel("PAYOUT_SIMULATED_SUCCESS"),
      "Payout simulation complete",
    );
    assert.equal(
      humanizePaymentLabel("IDR_FLOAT_CREDITED"),
      "Simulated IDR balance credited",
    );
  ...
  it("has English and Indonesian receipt translations", () => {
    ...
    assert.deepEqual(
      Object.keys(translations.en.receipt).sort(),
      Object.keys(translations.id.receipt).sort(),
    );
    assert.equal(
      translations.en.receipt.shareText,
      "Paid with KonekPay. Verified on Solana Devnet.",
    );
    assert.equal(
      translations.id.receipt.shareText,
      "Dibayar lewat KonekPay. Terverifikasi di Solana Devnet.",
    );
  });
});
